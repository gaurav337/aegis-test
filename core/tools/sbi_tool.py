...
class SBITool(BaseForensicTool):
    ...
    def _load_model(self) -> torch.nn.Module:
        ...
        model.classifier = nn.Sequential(
            nn.Dropout(p=0.4, inplace=True), nn.Linear(num_features, 1)
        )
    ...
    def _run_inference(self, input_data: dict) -> ToolResult:
        ...
        final_score = (
            ...
            if (
                ...
                or (
                    ...
                    and best_score > SBI_FAKE_THRESHOLD
                )
            )
            else 0.0
        )
        ...
        if boundary_detected:
            ...
            summary = (
                f"SBI detector: strong synthetic signatures detected globally (authenticity: {1.0 - final_score:.2f}), "
        ...
        confidence = (
            min(1.0, best_score + 0.2)
            if boundary_detected
            else max(0.5, 1.0 - abs(best_score - 0.5))
        )
