"""C2PA Provenance Tool — Content Credentials Verification (V2).
...
V2 Fixes:
    1. is_ai_generated now affects score (1.0 = fake when valid sig + AI declared)
    2. Action history parsed for manipulation severity scoring
    3. Absence of C2PA correctly abstains (no influence on ensemble)
    4. Case-insensitive pattern-based keyword matching + IPTC digitalSourceType
    5. Cryptographic signature validation depth checking
    6. Full provenance chain reported (creation tool + final tool)
"""
...
class C2PATool(BaseForensicTool):
    ...
    def _is_ai_keyword(self, text: str) -> bool:
        """Check if text contains AI-related keywords using regex patterns."""
    ...
    def _check_signature_validity(self, c2pa_dict: dict) -> Tuple[bool, str]:
        """Check cryptographic signature validation status.
        ...
        if validation_status:
            ...
            if "invalid" in validation_lower or "fail" in validation_lower:
                return False, f"Signature invalid: {validation_status}"
            if "untrusted" in validation_lower or "unknown" in validation_lower:
        ...
        if not cert_info and signer:
            return True, f"Signer: {signer} (certificate details unavailable)"
        ...
        try:
            ...
            if hasattr(c2pa, "read_file"):
                try:
                    ...
                    if any(
                        k in err_msg for k in ("not found", "no jumbf", "not supported")
                    ):
                        return self._no_c2pa_result(start_time)
                    raise read_err
            else:
            ...
            if sig_valid:
                ...
                if is_ai_generated:
                    ...
                    summary = (
                        ...
                        f"Provenance chain integrity uncertain."
                    )
