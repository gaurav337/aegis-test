...
class VRAMLockGuard:
    ...
    def _start_watchdog(cls):
        ...
        def _watch():
            ...
            if cls._lock_held:
                logger.critical(
                    "VRAM lock watchdog: forcing release due to stale hold."
                )
                try:
                    cls._lock.release()
                    cls._lock_held = False
...
class ToolRegistry:
            ...
            try:
                if label in self.GPU_TOOLS:
                    ...
                    if not health.success:
                        self.failed_tools[label] = (
                            f"Proxy setup failed: {health.error_msg}"
                        )
                        continue
    ...
    def execute_tool(self, name: str, input_data: dict) -> ToolResult:
        ...
        if tool is None:
            return ToolResult(
                ...
                execution_time=0.0,
                evidence_summary="Tool missing from registry.",
            )
        ...
        if not cb.should_allow():
            return ToolResult(
                ...
                execution_time=0.0,
                evidence_summary="Circuit breaker active.",
            )
            ...
            try:
                ...
                if attempt == max_retries:
                    ...
                    self._update_metrics(
                        name, time.perf_counter() - start, success=False
                    )
                    return ToolResult(
                        tool_name=name,
                        success=False,
                ...
                logger.warning(
                    f"Retrying {name} (attempt {attempt + 1}/{max_retries})..."
                )
    ...
    def get_health_report(self) -> dict:
        report = {
            ...
            "circuit_states": {
                n: cb.state.value for n, cb in self.circuit_breakers.items()
            },
            "execution_stats": {},
        }
