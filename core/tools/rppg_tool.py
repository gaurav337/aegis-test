"""rPPG Tool V2 — Biological Liveness Detection
...
class RPPGTool(BaseForensicTool):
    ...
    def _get_facial_rois(self, landmarks: np.ndarray) -> Dict[str, tuple]:
        ...
        if len(landmarks.shape) == 2 and landmarks.shape[0] == 478:
            ...
            if face_w > 0 and face_h > 0:
                ...
                rois["forehead"] = _rel(
                    landmarks[[109, 10, 338, 297, 332, 284, 103, 67]]
                )
                rois["left_cheek"] = _rel(landmarks[[50, 205, 207, 215, 138, 135, 210]])
                rois["right_cheek"] = _rel(
                    landmarks[[280, 425, 427, 435, 367, 364, 430]]
                )
        return rois
    ...
    def _check_hair_occlusion(self, roi: np.ndarray) -> Tuple[bool, float]:
        """Check hair occlusion using Laplacian variance + color histogram.
        ...
        # Additional check: color histogram chrominance difference
        # Hair typically has different chrominance than skin
        if roi.ndim == 3 and roi.shape[2] == 3:
            hsv = cv2.cvtColor(roi, cv2.COLOR_RGB2HSV)
            # Skin typically has H in 0-50 range, hair often outside
            h_channel = hsv[:, :, 0]
    ...
    def _detect_motion_contamination(
        ...
        """Detect if ROI signal is contaminated by head motion.

        Motion creates low-frequency variation (<0.5 Hz). If the majority
        of signal energy is below 0.5 Hz, the segment is motion-contaminated.
        Returns (is_contaminated, motion_ratio).
        """
        if len(rgb_means) < RPPG_MIN_FRAMES:
            return True, 1.0

        rgb_matrix = np.array(rgb_means, dtype=np.float64)
        # Use green channel for motion detection (highest SNR)
        signal = rgb_matrix[:, 1]
        ...
        # Motion energy: below 0.5 Hz
        motion_mask = freqs < 0.5
        cardiac_mask = (freqs >= RPPG_CARDIAC_BAND_MIN_HZ) & (
            freqs <= RPPG_CARDIAC_BAND_MAX_HZ
        )
        ...
        # If motion energy dominates cardiac energy by 3:1, flag as contaminated
        is_contaminated = motion_ratio > 0.75
        return is_contaminated, motion_ratio
    ...
    def _extract_pos_signal(
        ...
        Returns (H, temporal_std, hair_occluded, motion_contaminated)
        or (None, 0.0, False, False) on failure.
        """
        rgb_means = []
        last_known_box = None
        ...
        for f_idx, frame in enumerate(frames):
            ...
            # FIX 4: Check hair occlusion periodically (every 15 frames)
        ...
        # FIX 8: Check for motion contamination
        ...
        # Hair occlusion: if >40% of checked frames had hair, flag it
        hair_occluded = (
            total_checked_frames > 0
            and (hair_occlusion_frames / total_checked_frames) > 0.4
        )
    ...
    def _calculate_signal_metrics(
        ...
        band_mask = (freqs >= RPPG_CARDIAC_BAND_MIN_HZ) & (
            freqs <= RPPG_CARDIAC_BAND_MAX_HZ
        )
        ...
        # FIX 5: Use actual SNR for quality assessment
    ...
    # ─── FIX 1, 2, 3, 7: Conservative liveness evaluation ───
    ...
    def _evaluate_liveness(
        ...
        # FIX 1: Hair occlusion → abstention
        if hair_occluded:
            return {
                "label": "AMBIGUOUS",
                "score": 0.0,
                "confidence": 0.0,
                "interpretation": (
                    "rPPG abstained: Forehead ROI frequently occluded by hair. "
                    "Blood flow signal cannot be reliably extracted."
                ),
            }

        # FIX 8: Motion contamination → abstention
        if motion_contaminated:
            return {
                ...
                "confidence": 0.0,
                "interpretation": (
                    "rPPG abstained: Signal contaminated by head motion. "
                    "Cardiac pulse cannot be reliably distinguished from motion artifacts."
                ),
            }

        all_flat = all(np.std(s) < 1e-5 for s in signals)
        if all_flat:
            # FIX 1: Only flag as fake if signal quality is verified good
            # (well-lit, unoccluded, stable face)
            return {
                "label": "NO_PULSE",
                "score": 0.0,  # FIX 1: Abstention, not fake
                "confidence": 0.0,
                "interpretation": (
                    "Biological liveness inconclusive: All facial regions show minimal "
                    "temporal variation. This may indicate poor lighting, motion, or "
                    "static media — insufficient evidence for a liveness judgment."
                ),
            }

        # FIX 7: SYNTHETIC_FLATLINE → abstention (skin tone bias mitigation)
        analyzable_count = sum(1 for s in quality_stds if s >= RPPG_MIN_TEMPORAL_STD)

        if analyzable_count < 2:
            return {
                "label": "AMBIGUOUS",
                "score": 0.0,  # FIX 2, 7: Abstention, not fake
                "confidence": 0.0,
                "interpretation": (
                    f"rPPG abstained: Only {analyzable_count}/3 facial regions have sufficient "
                    f"temporal variance for analysis. This is common with directional lighting, "
                    f"shadows, or skin tone variation — insufficient evidence for liveness."
                ),
            }
        ...
        # FIX 5: Use actual SNR for quality filtering, not spectral concentration alone
        good_mask = [m["snr_db"] >= RPPG_SNR_THRESHOLD for m in metrics]
        n_good = sum(good_mask)

        # FIX 1: No good peaks → abstention, not fake
        if n_good == 0:
            ...
            return {
                ...
                "confidence": 0.0,
                "interpretation": (
                    f"rPPG abstained: No cardiac peak detected with sufficient SNR "
                    f"in any facial region (max SNR: {max_snr:.1f} dB). "
                    f"Signal quality insufficient for liveness judgment."
                ),
            }

        # FIX 2: Single ROI → abstention, not fake
        if n_good == 1:
            ...
            return {
                ...
                "confidence": 0.0,
                "interpretation": (
                    f"rPPG abstained: Only {roi_labels[good_idx]} yielded a usable signal. "
                    f"Multi-region coherence required for liveness confirmation."
                ),
            }

        # 2+ good ROIs — check coherence
        ...
        plausible_peaks = [
            p
            for p in good_peaks
            if (RPPG_HEART_RATE_MIN / 60.0) <= p <= (RPPG_HEART_RATE_MAX / 60.0)
        ]
        ...
        if not plausible_peaks:
            return {
                ...
                "confidence": 0.0,
                "interpretation": (
                    "rPPG abstained: Detected peaks outside plausible heart rate range "
                    f"({RPPG_HEART_RATE_MIN}-{RPPG_HEART_RATE_MAX} BPM). "
                    "Likely motion artifact or noise."
                ),
            }
        ...
        if best_pair_diff <= RPPG_COHERENCE_THRESHOLD_HZ:
            pair_avg_hz = (
                metrics[best_pair[0]]["peak_hz"] + metrics[best_pair[1]]["peak_hz"]
            ) / 2
            n_coherent = sum(
                1
                for i in good_indices
                if abs(metrics[i]["peak_hz"] - pair_avg_hz)
                <= RPPG_COHERENCE_THRESHOLD_HZ
            )
        ...
        # FIX 1: Incoherent peaks → abstention, not fake
        ...
        return {
            ...
            "confidence": conf,
            "interpretation": (
                f"rPPG abstained: Pulse frequencies vary across facial regions "
                f"(best pair spread: {best_pair_diff * 60:.1f} BPM). "
                f"May indicate motion artifacts or poor signal quality."
            ),
        }
    ...
    def _run_inference(self, input_data: Dict[str, Any]) -> ToolResult:
        ...
        # Check if this is a static image (rPPG requires video)
        ...
        # FIX 6: Removed lightweight face check — it was a dead end
        ...
        for face in tracked_faces:
            ...
            h_forehead, std_f, hair_f, motion_f = self._extract_pos_signal(
                target_frames, sliced_trajectory, rois["forehead"]
            )
            h_left, std_l, hair_l, motion_l = self._extract_pos_signal(
                target_frames, sliced_trajectory, rois["left_cheek"]
            )
            h_right, std_r, hair_r, motion_r = self._extract_pos_signal(
                target_frames, sliced_trajectory, rois["right_cheek"]
            )

            # FIX 3: Tracking failure → abstention, not fake
            if h_forehead is None or h_left is None or h_right is None:
                if hair_f or hair_l or hair_r:
                    face_results.append(
                        {
                            "score": 0.0,
                            "confidence": 0.0,
                    ...
                    # FIX 3: Tracking failure is always an abstention
                    face_results.append(
                        {
                            "score": 0.0,
                            "confidence": 0.0,
                            "label": "ABSTAIN",
                            "interpretation": (
                                "rPPG abstained: Facial tracking was unstable. "
                                "Cannot extract reliable biological signal."
                            ),
                            "metrics": {},
                        }
                    )
            ...
            liveness = self._evaluate_liveness(
                ...
                motion_contaminated=motion_f,
            )

            metrics = [
                self._calculate_signal_metrics(s) for s in [h_forehead, h_left, h_right]
            ]
            face_results.append(
                {
        ...
        # Use highest-confidence result (but all should be 0.0 for abstentions)
        best = sorted(face_results, key=lambda x: x["confidence"], reverse=True)[0]

        details = {
            "liveness_label": best["label"],
            "peak_hz": best["metrics"].get("forehead", {}).get("peak_hz", 0.0),
            "spectral_concentration": best["metrics"]
            .get("forehead", {})
            .get("spectral_concentration", 0.0),
