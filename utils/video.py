...
acceleration (NVDEC via torchcodec) when available, falling back to OpenCV with
automatic FFmpeg transcoding for unsupported codecs (AV1, HEVC, VP9, etc.).
...
def _calculate_scale(
    width: int, height: int, max_width: int = 1280
...
def _transcode_to_h264(video_path: str) -> Optional[str]:
    """Transcode a video to H.264/MP4 using FFmpeg for compatibility.

    Handles codecs like AV1, HEVC, VP9 that may not be supported by the
    hardware or OpenCV. Returns path to the transcoded temp file, or None.
    """
    if not _has_ffmpeg():
        logger.error("FFmpeg not found on system. Cannot transcode unsupported codec.")
        return None

    temp_dir = tempfile.mkdtemp(prefix="aegis_transcode_")
    output_path = os.path.join(temp_dir, "transcoded_h264.mp4")
    ...
    logger.info(f"Transcoding video to H.264 for compatibility: {video_path}")
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
        if result.returncode != 0:
            logger.error(f"FFmpeg transcoding failed: {result.stderr[:500]}")
            return None

        if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
            logger.error("FFmpeg transcoding produced no output file.")
            return None

        logger.info(f"Video transcoded successfully to H.264: {output_path}")
        return output_path
    except subprocess.TimeoutExpired:
        logger.error("FFmpeg transcoding timed out (300s).")
        return None
    except Exception as e:
        logger.error(f"FFmpeg transcoding exception: {e}")
        return None
...
def extract_frames(
    ...
    CODEC SAFETY: Automatically transcodes unsupported codecs (AV1, etc.) to H.264.
    """
    ...
    if TORCHCODEC_AVAILABLE and torch and torch.cuda.is_available():
        ...
        if total_vram_gb >= VRAM_MODEL_LOAD_THRESHOLD:
            use_gpu_decode = True
            logger.info(
                f"VRAM detected: {total_vram_gb:.1f}GB (threshold: {VRAM_MODEL_LOAD_THRESHOLD}GB). Using GPU decoding (NVDEC)."
            )
        else:
            logger.warning(
                f"VRAM detected: {total_vram_gb:.1f}GB (threshold: {VRAM_MODEL_LOAD_THRESHOLD}GB). Forcing CPU decoding to reserve VRAM for models."
            )
    ...
    if TORCHCODEC_AVAILABLE:
        try:
            ...
            batch_size = (
                GPU_DECODE_BATCH_SIZE if use_gpu_decode else CPU_DECODE_BATCH_SIZE
            )
            ...
            for i in range(0, len(indices), batch_size):
                ...
                for frame in frames_np:
                    ...
                    if scale_dims:
                        frame = cv2.resize(
                            frame, scale_dims, interpolation=cv2.INTER_AREA
                        )
                        logger.warning(
                            f"Frame downscaled to {scale_dims} — may reduce forensic accuracy."
                        )
            ...
            logger.warning(
                f"TorchCodec extraction failed: {e}. Falling back to OpenCV CPU decode."
            )

    # Fallback path: OpenCV CPU Decoding (with FFmpeg transcoding if needed)
...
def _extract_cv2(video_path: str, max_frames: int, target_fps: int) -> List[np.ndarray]:
    ...
    If OpenCV cannot read the video (e.g., unsupported codec like AV1),
    automatically transcodes to H.264 via FFmpeg and retries.
    """
    frames = _extract_cv2_internal(video_path, max_frames, target_fps)

    # If OpenCV failed to extract any frames, try FFmpeg transcoding
    if len(frames) == 0:
        logger.warning(
            f"OpenCV extracted 0 frames from {video_path}. "
            f"Attempting FFmpeg transcoding to H.264..."
        )
        transcoded_path = _transcode_to_h264(video_path)
        if transcoded_path:
            try:
                frames = _extract_cv2_internal(transcoded_path, max_frames, target_fps)
                if len(frames) > 0:
                    logger.info(
                        f"Successfully extracted {len(frames)} frames from transcoded video."
                    )
                else:
                    logger.error("Even transcoded video yielded 0 frames.")
            finally:
                # Clean up the transcoded temp file
                try:
                    shutil.rmtree(os.path.dirname(transcoded_path))
                except Exception:
                    pass
        else:
            logger.error(
                "FFmpeg transcoding failed or unavailable. Cannot extract frames."
            )

    return frames


def _extract_cv2_internal(
    video_path: str, max_frames: int, target_fps: int
) -> List[np.ndarray]:
    """Internal OpenCV frame extraction. Returns empty list on failure."""
    ...
    if source_fps <= 0 or math.isnan(source_fps):
        ...
        logger.warning(
            f"OpenCV returned invalid FPS ({source_fps}), using fallback {FALLBACK_FPS}"
        )
        ...
        if frame_idx >= int(round(current_target)):
            ...
            if scale_dims:
                frame_bgr = cv2.resize(
                    frame_bgr, scale_dims, interpolation=cv2.INTER_AREA
                )
                logger.warning(
                    f"Frame downscaled to {scale_dims} — may reduce forensic accuracy."
                )
